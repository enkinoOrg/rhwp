...
## createEditor 초기화 실패 cleanup

### 배경과 원인

`createEditor()`는 iframe을 붙이고 `RhwpEditor`의 message listener를 등록한 뒤 `_waitReady()`를 호출한다. 성공 반환 전 editor 소유권은 호출자에게 전달되지 않으므로, `_waitReady()`가 실패하면 호출자가 `destroy()`할 방법 없이 iframe, listener, pending timer가 남았다.

### TDD

ready 전송 실패로 pending timer가 남은 상태에서 retry 대기가 원 오류로 실패하는 실행 테스트를 추가했다.

RED에서 원 오류 객체는 그대로 전달됐지만 cleanup 상태는 다음과 같았다.

- active timer: `1`
- iframe 제거: `0`
- message listener: `1`

GREEN에서는 `_waitReady()` 경계에 `try/catch`를 두고, 실패 시 `editor.destroy()`를 호출한 뒤 잡은 오류 객체를 그대로 다시 throw한다. 실행 테스트는 다음을 함께 확인한다.

- 원 오류 객체 identity 보존
- iframe 제거 1회
- message listener 0개
- active pending timer 0개

### 검증

- `node --test npm/editor/index.test.mjs`: 6개 통과
