...
- 결과 HWPX 경로와 SHA-256
- 비교 문서 자체는 Git이 버전을 추적하며, 필요할 때 저장소에서 blob ID와 포함 커밋을 조회한다.
- 보존 마지막 앵커와 교체 첫 앵커
...
- 비교 문서는 입력, 템플릿, 결과 파일의 정확한 경로와 SHA-256을 기록한다. 비교 문서 자체는 자기참조 SHA-256·blob ID·커밋 ID를 내부에 기록하지 않고 Git 이력으로 버전을 식별한다.
