...
현재 npm 발행 권한이 연결되지 않았으므로 보안 SDK를 고정 커밋에서 vendor한다.
...
iframe 메시지 origin/source 검증, pending 요청 정리, transferable 바이너리 프로토콜이 반영된 기준은 커밋 `9fc2bcbda1f5787c60b89244d01b4ff80e3adeab`이다. `@rhwp/editor@0.7.19`가 npm에 실제 발행된 것을 확인한 뒤에는 package 설치로 바꿀 수 있다.
