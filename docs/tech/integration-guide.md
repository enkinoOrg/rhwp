...
6. repository가 새 object를 만든 후 append-only version 이력과 현재 version을 한 트랜잭션으로 갱신한다. commit 응답이 유실되면 DB에서 `storagePath`와 version의 참조 여부를 재조회한다.
...
iframe 메시지 origin/source 검증과 pending 요청 정리가 반영된 SDK 최소 버전은 `0.7.19`다.
...
Route는 [validate-hwpx-archive.ts](../../examples/nextjs-integration/server/validate-hwpx-archive.ts)의 `createHwpxArchiveValidator`에 프로젝트의 `@/server/security/zip-inspector` adapter를 주입한다. adapter는 proven ZIP library를 사용해 중앙 directory와 제한된 실제 압축 해제를 검사하고 `ZipInspector` interface를 구현해야 한다. 예제 안에서 ZIP binary parser를 직접 작성하거나 검사 없는 stub을 production에 두지 않는다.
...
경쟁 저장으로 RPC가 conflict를 돌려주면 repository는 방금 만든 미참조 orphan object만 삭제한다. 반면 `commitNewVersion` throw는 commit 실패가 아니라 **commit-unknown**이다. [document-repository.ts](../../examples/nextjs-integration/server/document-repository.ts)는 `resolveVersionCommit`으로 document row lock 뒤 `(documentId, version, storagePath)`를 재조회한다. `committed`면 PUT 성공을 복구하고, `not-committed`일 때만 cleanup하며, 조회 실패나 `unknown`은 object를 보존하고 durable GC queue에 기록한다.

Supabase RPC `resolve_document_version_commit`은 선행 commit RPC와 같은 document row를 잠가 선행 트랜잭션 종료를 기다린다. [document-storage-gc.ts](../../examples/nextjs-integration/server/document-storage-gc.ts)의 `collectDocumentStorageGarbage`도 삭제 직전에 이 API를 다시 호출한다. `not-committed`만 삭제하고, `committed`는 queue를 완료 처리하며, 조회 실패/`unknown`은 보존한다. 이것이 GC의 executable safety contract다.
...
[validate-hwpx-archive.ts](../../examples/nextjs-integration/server/validate-hwpx-archive.ts)는 Storage 저장 전에 entry count, 안전한 경로, 중복 entry, 필수 HWPX entry, `application/hwp+zip` mimetype, entry별/전체 uncompressed 크기와 XML 크기를 fail-closed로 제한한다. inspector 자체도 metadata만 신뢰하지 않고 실제 압축 해제 byte 수를 제한해야 한다. 검증된 ZIP library adapter가 암호화 entry, 지원하지 않는 압축 방식, 중앙 directory 불일치를 거부하고 제한된 `readText`를 제공한다.
...
**축약 예제**: 패키지 설치, 복사 위치, bucket 이름 변경 지점, SQL 적용 전 검토는 [Next.js 예제 README](../../examples/nextjs-integration/README.md)를 따른다. `@rhwp/editor@^0.7.19`와 `@supabase/supabase-js`를 설치하고, `route.ts`의 인증 helper 및 ZIP inspector import를 프로젝트 adapter로 연결한다. SQL은 예제이므로 기존 테이블, 사용자 ID 타입, RLS와 service_role 권한을 검토한 뒤 적용한다.
...
- `commit-unknown` queue를 모니터링하고 GC가 삭제 직전 DB 참조를 재조회하는지 확인한다.
