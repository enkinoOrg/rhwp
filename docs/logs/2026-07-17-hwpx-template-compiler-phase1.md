...
- Phase 1은 경계가 속한 구역 뒤의 구역 노드를 삭제하지 않고 문단만 비워 둔다. 이는 구역 인덱스와 구역 단위 설정을 보존하기 위한 의도된 정책이며, 후속 빈 구역이 렌더러에서 빈 페이지로 보일 가능성은 Phase 2에서 별도 모델링한다.
- 현재 통합 fixture는 마지막 구역(`1:36`)을 패치하므로 후속 빈 구역이 없다. patch/export/reload 뒤 구역 수가 canonical 기준선과 같고 모든 구역에 문단이 남아 있음을 검사해 예상 밖 빈 페이지 추가를 막는다.
...
- 경계 의미를 좌표만으로 추론하지 않도록 `BoundaryCandidate`에 보존되는 마지막 앵커와 교체되는 첫 앵커를 명시하는 필드를 설계한다.
- 이 필드는 호출자·직렬화 형식과의 호환성 검토가 필요한 Phase 2 공개 API 작업이다. Phase 1 공개 타입에는 계획 없이 필드를 추가하지 않는다.
